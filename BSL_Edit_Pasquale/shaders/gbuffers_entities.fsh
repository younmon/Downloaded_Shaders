...
void main(){
	...
	if (albedo.a > 0.0){
		...
		if (dot(fullshading,fullshading) > 0.0){
			...
			spec = pow(speccol,vec3(1.0-0.5*f0)) * max(vec3(shadow), shadowcol) * clamp(NdotL*100.0,0.0,1.0) * shadowFade * skymap * metalcol * (1.0 - rainStrength);
			spec *= GGX(normal,normalize(fragpos.xyz),lightVec,1.0-0.98*smoothness,f0*0.785+0.02);
			albedo.rgb += spec;
		}
		#endif
	}
