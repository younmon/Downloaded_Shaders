...
#define POMQuality 64
#define POMDepth 0.80 
#define POMDistance 32.0 
#define POMShadowAngle 2.0 
#define RPSupport
#define RPSFormat 1 
//#define RPSPom
...
float waterH(vec3 pos, vec3 fpos) {
	...
	if (mult > 0.01){
		#if WaterNormals == 1
		noise  = texture2D(noisetex,(pos.xz+vec2(frametime)*0.5-pos.y*0.2)/256.0* 1.1).r*1.0;
		noise += texture2D(noisetex,(pos.xz-vec2(frametime)*0.5-pos.y*0.2)/256.0* 1.5).r*0.8;
		noise -= texture2D(noisetex,(pos.xz+vec2(frametime)*0.5+pos.y*0.2)/256.0* 2.5).r*0.6;
		noise += texture2D(noisetex,(pos.xz-vec2(frametime)*0.5-pos.y*0.2)/256.0* 5.0).r*0.4;
		noise -= texture2D(noisetex,(pos.xz+vec2(frametime)*0.5+pos.y*0.2)/256.0* 8.0).r*0.2;
		noise *= mult;
		#endif
		#if WaterNormals == 2
...
void main(){
	...
	if (albedo.a > 0.0){
		...
		if (water < 0.5) albedo.rgb /= sqrt(albedo.rgb * albedo.rgb + 1.0);
		...
		if (water > 0.5 || (translucent > 0.5 && albedo.a < 0.95)){
			...
			if (reflection.a < 1.0){
				...
				float sun = GGX(newnormal,normalize(fragpos.xyz),lightVec,0.05,0.02) * (1.0-sqrt(rainStrength)) * shadowFade;
				...
				skyRef += (sun / ((4.0-3.0*eBS) * fresnel)) * speccol * max(vec3(shadow),shadowcol);
				#ifdef Clouds
			...
			if(fresnel > 0.001){
				...
				if (f0 >= 0.8) reflection.rgb *= albedo.rgb * 2.0;
			...
			if (dot(fullshading,fullshading) > 0.0){
				vec3 metalcol = mix(vec3(1.0),pow(rprawalbedo,vec3(2.2)),float(f0 >= 0.8));
				vec3 light_me = mix(light_m,light_a,mefade);
				vec3 speccol = mix(sqrt(light_n),mix(light_me,sqrt(light_d*light_me),timeBrightness),sunVisibility);

				vec3 spec = pow(speccol,vec3(1.0-0.5*f0)) * max(vec3(shadow), shadowcol) * clamp(NdotL*100.0,0.0,1.0) * shadowFade * skymap * metalcol * (1.0 - rainStrength);
