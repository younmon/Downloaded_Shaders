...
void main(){
	...
	//Dither
	#if defined AO || defined EnableVL
	...
	//Black Outline
	#ifdef Celshade
	if (celshademask(depthtex0,depthtex1) > 0.5 || isEyeInWater > 0.5){
		float forcefull = float(z >= 1.0)*(1.0-0.5*rainStrength)*(1.0-isEyeInWater);
		color.rgb *= celshade(depthtex0, forcefull);
	}
	#endif
