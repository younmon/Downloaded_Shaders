...
void main(){
	...
	if (istopv > 0.9) {
		...
		if (mc_Entity.x == 31.0){
			...
			position.xyz += calcMove(worldpos.xyz, 0.0041, 0.0070, 0.0044, 0.0038, 0.0063, 0.0000, vec3(0.3,0.0,0.6), vec3(0.2,0.0,1.0));
		}
	#endif
		#ifdef WavingGrass
		if (mc_Entity.x == 32.0){
			if (length(position.xyz) < 2.0) position.xz *= 1+max(5.5/pow(max(length(position.xyz*vec3(8.0,2.0,8.0)-vec3(0.0,2.0,0.0)),2.0),1.0)-0.625,0.0);
			position.xyz += calcMove(worldpos.xyz, 0.0041, 0.0070, 0.0044, 0.0038, 0.0063, 0.0000, vec3(0.25,0.15,0.25), vec3(0.2,0.05,0.2));
		}
	#endif
	#ifdef WavingPlant
		if (mc_Entity.x == 6.0)
	...
	#ifdef WavingLeaves
	if (mc_Entity.x == 18.0)
		position.xyz += calcMove(worldpos.xyz, 0.0040, 0.0064, 0.0043, 0.0035, 0.0037, 0.0041, vec3(0.25,0.25,0.25), vec3(0.15,0.15,0.15));
	#endif
	#ifdef WavingLeaves
	if (mc_Entity.x == 34.0)
		position.xyz += calcMove(worldpos.xyz, 0.0040, 0.0064, 0.0043, 0.0035, 0.0037, 0.0041, vec3(0.05,0.1,0.05), vec3(0.05,0.1,0.05));
	#endif
	#ifdef WavingLeaves
	if (mc_Entity.x == 33.0)
		position.xyz += calcMove(worldpos.xyz, 0.0040, 0.0064, 0.0043, 0.0035, 0.0037, 0.0041, vec3(0.07,0.05,0.07), vec3(0.07,0.05,0.07));
	#endif
	#ifdef WavingVines
	if (mc_Entity.x == 106.0)
		position.xyz += calcMove(worldpos.xyz, 0.0040, 0.0064, 0.0043, 0.0035, 0.0037, 0.0041, vec3(0.02,0.15,0.02), vec3(0.02,0.15,0.02));
	if (mc_Entity.x == 35.0)
	position.xyz += calcMove(worldpos.xyz, 0.0040, 0.0064, 0.0043, 0.0035, 0.0037, 0.0041, vec3(0.02,0.05,0.08), vec3(0.02,0.05,0.08));
	#endif
	#ifdef WavingLilypad
	...
	//Foliage
	if (mc_Entity.x == 31.0 || mc_Entity.x == 32.0 || mc_Entity.x == 35.0 || mc_Entity.x == 33.0 || mc_Entity.x == 34.0 || mc_Entity.x == 6.0 || mc_Entity.x == 59.0 || mc_Entity.x == 175.0 || mc_Entity.x == 176.0 || mc_Entity.x == 18.0 || mc_Entity.x == 106.0 || mc_Entity.x == 111.0 || mc_Entity.x == 83.0)
	mat = 1.0;
	//Emissive
	...
	color = gl_Color;
	color.a = floor(color.a*4.0)/4.0;
	
	lmcoord = (gl_TextureMatrix[1] * gl_MultiTexCoord1).xy;
	lmcoord = clamp((lmcoord - 0.03125) * 1.06667, 0.0, 1.0);

	//specvalue = 1.0-floor(-(worldpos.xz-1.0)/4.0)*0.1;
	//if(worldpos.x > 1) specvalue = vec2(0);
